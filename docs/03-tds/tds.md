...
4. `별 ↔ 용돈 저축` 간 전환 필드나 전환 함수는 두지 않는다.
5. 위치정보는 애초에 스키마·권한·네트워크 인터페이스에 존재하지 않도록 설계한다.
6. `계획 카드 → 결제 원장 → 매칭 → 회고 → 실천 인정 → 별 지급/나무 갱신`은 핵심 도메인 흐름이다.
7. 성장 나무와 월간 숲은 별 잔액과 분리한다.
8. 실천 트리거는 MVP 기준 미션·소비 회고·위시리스트 저축 세 경로만 WPA에 포함한다.
9. 위시리스트 저축은 부모 용돈을 기록하는 별도 원장이고, 별 원장과 금액을 섞지 않는다.
10. 보호자 승인 지연은 실패가 아니라 정상적인 비동기 상태로 취급하고, 완료 시점과 지급 시점을 분리한다.
11. 분석 이벤트는 `idempotency_key`, `client_ts`, `server_ts`를 필수화하고 KST/ISO 주 기준으로 집계한다.
...
| UC-07 | 위시리스트 용돈 저축 | 아동/보호자 | 부모 용돈 저축 기록과 목표 진척에 따른 실천 인정 |
...
성장 나무는 `earn(벌기)`, `spend_well(쓰기)`, `save(모으기)`, `grow(불리기)` 네 슬롯으로 분리한다. 각 슬롯은 같은 4단계 시각 언어를 쓰되, 실천 카운터는 해당 영역에서만 올린다.
...
해당 영역 실천 0건이면 학습·퀴즈가 충분해도 그 나무는 승급하지 않는다. MVP에서 `grow(불리기)`는 학습·퀴즈 화면만 열고 실천 슬롯은 잠금 상태로 표시한다.
...
- child_id
- amount (Int, 원)
- source (`PARENT_ALLOWANCE` 고정)
- idempotency_key
- created_at

부모가 준 용돈을 위시리스트 목표에 넣었다는 사실만 기록한다. 별 원장과 전환하거나 현금 잔액으로 취급하지 않는다.
