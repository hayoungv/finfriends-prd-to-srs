...
| 버전 | 「실천 트리거」 범위 | 언제 쓰는가 |
|:-:|---|---|
| 🔴 **WPA-v1** *(MVP 기준)* | **트리거 4** 미션 승인 · **5** 소비 회고 · **6** 위시리스트 용돈 저축 — **3종** | v1.0 MVP 전 기간 |
...
| **F12** | 🎯 위시리스트 용돈 저축 (30·70·100% 각 ⭐1) | ★ C2 | J1 | 4 | 2 | **Should** | **F4** | **0.5스프린트** | 부모 용돈을 목표에 모아 「모으기」 칸을 여는 실천 · 저축에 **이유**를 만듦 |
...
| **실천 판정** | 아동_id · 경로(미션/회고/위시리스트 용돈 저축/예적금) · 판정 시각 · **판정 방식(자동/부모승인)** · 승인 지연 시간 · `cycle_id` | **WPA 산출의 원천 테이블.** 「사전개입」 경로 삭제 |
| **별 원장** | 아동_id · 증감 · 트리거 코드(1~8) · 잔액 · 사유 · `idempotency_key` | 🔴 **이중 기입 · 초기화 없음 · 현금 전환 필드 부재**(P-01·P-21) |
| **나무 상태** | 아동_id · 칸(벌기/쓰기/모으기/불리기) · 단계 · 영역별 조건 충족(학습/퀴즈/실천) · 주기 시작일 · **정체 일수** | 정체 일수가 **F1 정체 원인 표시**의 입력 |
| **위시리스트 저축 기록** | 위시리스트_id · 아동_id · 부모 용돈 적립액(원) · 출처(`PARENT_ALLOWANCE`) · `idempotency_key` | 별 원장과 분리된 「모으기」 실천 근거 |
...
| | 내용 |
|---|---|
| **In** *(이번 릴리즈 · Must 9 + Should 5)* | 부모가 준 용돈을 목표에 기록하는 **모으기** · 앱으로 충전된 용돈의 흐름 — **벌기 → 잘 쓰기 → 모으기** 3칸 개통 · **불리기**는 학습만 개통 · 학습 4주제 + 퀴즈 · 실천 판정 3종(미션·소비 회고·위시리스트 용돈 저축) · **소비 계획 카드 작성(F8a)** · **계획↔실제 업종별 대조(F8b)** · 별·아바타·옷장 · 성장 나무 · 월간 숲 · 소비 내역(업종별 집계) · 부모 온보딩 5단계·법정대리인 동의 · ⭐ 소급 지급 · 3일 미접속 알림 |
| **Out** *(제품 범위 밖 · 영구)* | **부모가 기록하지 않는 앱 밖 현금**(세뱃돈·친척 용돈) · **친구 기능** · 모의투자 *(퍼핀 앱 이름에 포함돼 넣으면 클론)* · 미션 사진 인증 *(퍼핀 동일 기능 + 아동 이미지 리스크)* · **위치정보 수집 일체**(ADR-003) |
...
flowchart TD
    ...
    T -->|"6 부모 용돈 저축 30·70·100%"| A6["각 별 1 · 자동<br/>모으기"]
