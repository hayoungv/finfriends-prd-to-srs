...
| 트랙 A/B/C/D 에이전트 | §5.4 소유권 제안 → §6 fixture 치환 규약 |
...
> `/child/tree`·`/parent/forest` 2건만 `TASK-403` 산출 파일 경로(`app/child/tree/loading.tsx`, `app/parent/forest/loading.tsx`)에서 **확정**된 라우트다. 나머지 `[제안]` 표기 라우트는 본 문서가 처음 제안하는 것으로, 승인 시 `AGENTS.md` §3 디렉토리 규약에 반영해야 한다.
...
| 2 | `/child/learn` `[제안]` | Fun | `206` | REQ-FUNC-003 | 4주제 카드(벌기/쓰기/모으기/불리기), **'불리기' 잠금 표시**(AC1 · ADR-006 · REG-004) |
| 3 | `/child/quiz/[topic]` `[제안]` | Fun | `206` | REQ-FUNC-003 AC2 | 정답 제출 → **별 1개 즉시 지급 피드백** |
| 4 | `/child/missions` `[제안]` | Fun | `207` | REQ-FUNC-004 | 미션 상태 전이 시각화 (`CREATED → PENDING_APPROVAL → APPROVED/REJECTED`) |
| 5 | `/child/plan/new` `[제안]` | Fun | `209` | REQ-FUNC-007 | 3필드 폼(장소·업종·금액), 72시간 만료 표시, **위치/카메라 권한 UI 부재**(AC2 · REG-002) |
| 6 | `/child/retro/[recordId]` `[제안]` | Fun | `211` | REQ-FUNC-008 | **3상태** — AI칭찬(별 지급) / AI격려(별 미지급) / 룰Fallback |
| 7 | `/child/stars` `[제안]` + `StarHUD` | Fun | `205` | REQ-FUNC-002 | 잔액 카운터, 획득 이력 페이징. **별↔현금 전환 UI 부재**(REG-005c) |
| 8 | `/child/wardrobe` `[제안]` | Fun | `214` | REQ-FUNC-006 | 아바타 2종·의상 4종 그리드, 잔액 부족 시 구매 차단 상태, **얼굴 업로드 UI 부재**(AC3 · REG-006) |
| 9 | `/child/wishlist` `[제안]` | Fun | `215` | REQ-FUNC-013 | 30/70/100% 마일스톤 게이지 (중복 지급 금지 표시) |
| 10 | `/parent/onboarding` `[제안]` | Clean | `201` | REQ-FUNC-001 AC1 | 5단계 스텝 인디케이터, **중간 이탈 후 재개** 상태 |
| 11 | `/consent` `[제안]` | Clean | `202` `203` | REG-001 | 동의 폼 + 아동 프로필 생성. 미동의 시 리다이렉트 착지점 |
| 12 | `/parent/forest` | Clean | `213` `403` `404` | REQ-FUNC-009 | **7대 지표** 리포트, 최상단 미접속 배너, 스켈레톤 |
| 13 | `/parent/missions` `[제안]` | Clean | `207` `208` | REQ-FUNC-011 AC3 | 승인 대기 목록 + **"한 번에 모두 칭찬하기"** 버튼(5건 이상 시) |
...
제안 — **화면 소유권을 해당 Server Action 소유 트랙에 정렬**한다:
...
| ID | 항목 | 영향 | 대응 |
|:--:|---|:--:|---|
| **R-P1** | **DAG에 화면 생성 태스크가 없다** — `305`·`306` E2E가 존재하지 않는 화면을 대상으로 착수 | 치명 | 본 계획 승인 또는 UI 태스크 신설. **둘 중 하나를 Wave 0 이전에 결정** |
| **R-P2** | `app/**`·`components/child/**` **소유권 미배정** — 4트랙 병렬 시 충돌 | 높음 | §5.4 제안 승인 후 `AGENTS.md` §6 · 로드맵 §2.3 동시 갱신 |
| **R-P3** | `[제안]` 라우트 11건이 어느 문서에도 없음 | 중 | 승인 시 `AGENTS.md` §3 디렉토리 규약에 반영. 미승인 시 프로토타입 착수 보류 |
| **R-P4** | 프로토타입 fixture 타입이 `TASK-102` DTO와 불일치 | 중 | fixture는 SRS §10·§11 기준으로 **선취 정의**하고, `102` 완료 시 `types/domain.ts`로 역흡수 |
| **R-P5** | 프로토타입 2.5 MD가 임계 경로(6.0 MD)에 순증 | 중 | P0~P1(1.5 MD)을 Wave 0 유휴에 흡수(§5.1·§5.2). 실질 순증 **약 1.0 MD** |
| **R-P6** | 화면 존재가 REG-001 통과로 오인 | 높음 | §5.5 명시. Alpha Gate 판정은 `TASK-203`+`TASK-305` 완료 시에만 유효 |

### 미결정 사항 (오케스트레이터 판단 필요)

1. **본 계획을 별도 태스크(`TASK-100` 등)로 티켓화할 것인가**, 아니면 Wave 0 준비 작업으로만 둘 것인가
2. **§5.4 소유권 배정안 승인 여부** — 대안으로 "UI 전담 5번째 트랙(E) 신설"도 가능하나, 4트랙 부하 균등(5.0~5.5 MD) 설계가 재산출 대상이 된다
3. **`[제안]` 라우트 11건 명명 확정** — 특히 `/consent`를 `app/consent/`(루트)에 둘지 `app/parent/consent/`에 둘지. REG-001 리다이렉트 대상이 `/consent`로 명시되어 있어 **루트 배치를 권장**
