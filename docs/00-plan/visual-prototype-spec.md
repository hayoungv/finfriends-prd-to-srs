...
> ⚠️ **2026-08-27 폐기.** Node LTS v24.20.0 이 설치되어 이 문서의 전제("Node 가 없다")가 소멸했다.
> 후속 지시서는 [`prototype-execution-plan.md`](prototype-execution-plan.md) 다. **착수는 그쪽을 읽는다.**
>
> | 이 문서의 절 | 승계처 | 비고 |
> |---|---|---|
> | §4 `tokens.css` 값 | 새 문서 §5 | hex·px·ms 그대로. 그릇만 `@theme` 으로 바뀐다 |
> | §2 프레임 명세 15건 | 새 문서 §4.1 | 라우트 6건으로 매핑. **§2.2 주제 목록은 `/child/learn` 으로 분리 교정** |
> | §7 완료 판정 6문항 | 새 문서 §8 | 그대로 + 클릭 완주·빌드 3항 추가 |
> | §6 불변식 | 새 문서 §7 | 면제 없음 |
> | §3 모드 접근 (토글 없음) | 새 문서 §5 | 유효 |
>
> 정적 HTML 목업(`docs/00-plan/mockups/`)은 **만들지 않는다.** 같은 판단을 실제 `app/**` 에서 한다.
