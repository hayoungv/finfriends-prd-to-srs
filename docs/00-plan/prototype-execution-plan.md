...
- **대체 대상:** [`visual-prototype-scope.md`](visual-prototype-scope.md) (전제 소멸) · [`visual-prototype-spec.md`](visual-prototype-spec.md) (프레임 명세는 §4 로 승계)
...
> ⚠️ **정적판의 오류 1건을 여기서 교정한다.** `visual-prototype-spec.md` §2.2 는 주제 목록을
...
| 13 | `/parent/missions` 은 §4.1 에 포함 · `/child/learn` 도 포함 | — | — | — | — |
...
- `app/layout.tsx` 가 `<html data-mode="fun|clean">` 를 세그먼트 기준으로 부여한다. **토글 UI 는 만들지 않는다** (`visual-prototype-spec.md` §3 — 계정 분리 + 부모→아이 단방향).
- 별 획득 모션은 CSS `@keyframes` + `prefers-reduced-motion` 존중. 나무 승급은 정지 이미지 교체.
