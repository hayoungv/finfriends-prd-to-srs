...
| 영역별 게이지 | 벌기·쓰기·모으기·불리기 나무를 한 장씩 스와이프 · 현재 영역의 학습/퀴즈/실천 근거 |
...
| **R-P1** | DAG 에 화면 생성 태스크가 없다 | ✅ **해소** | grill T7 · 안 A — 12개 태스크 명세에 화면 선작성 절 삽입. 새 티켓 0건 |
...
**R-P1 은 2026-08-27 grill 세션 2 T7 에서 안 A 로 해소됐다.**

- P0 는 `TASK-101` 의 스캐폴딩 구간으로 흡수한다 (`CLAUDE.md` §1 무충돌).
- P1~P3 의 각 화면은 `AGENTS.md` §6 소유 트랙의 **해당 태스크가 선행 작업으로 수행**한다. 새 티켓 0건, DAG 30건 불변.
- 12개 태스크 명세에 `### 🎨 [PROTO] 화면 선작성` 절을 삽입해 책임을 성문화했다 — `101` `201` `202` `205` `206` `207` `209` `211` `212` `213` `214` `215`.

```bash
# 화면 책임이 성문화된 태스크 전수 확인 — 12건
grep -rl "PROTO. 화면 선작성\|PROTO. P0 스캐폴딩" tasks/ | sort
```
...
| `tasks/step-*/TASK-XXX.md` | **갱신됨** | 12건에 `[PROTO] 화면 선작성` 절 추가 (grill T7 · 안 A) |
| `visual-prototype-scope.md` · `visual-prototype-spec.md` | **삭제됨** | 2026-08-27 저장소에서 제거. 화면 명세 → §4.3 · 모드 접근 → §4.4 · 토큰 → §5 · 모션 → §5.1 · 완료 판정 → §8 로 전부 승계 |
| `docs/goals/prototype-node-aztks-gate.md` | **유효** | 이 지시서를 구동하는 `/goal` 원본. 실행 시각 스냅샷은 같은 디렉토리의 타임스탬프 파일 |
