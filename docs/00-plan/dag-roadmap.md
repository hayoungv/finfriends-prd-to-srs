...
> 🎯 **최우선 관리 노드는 `TASK-212`(4영역 성장 나무의 영역별 3조건 판정)입니다.** 세 체인이 모두 이 노드를 통과하므로, `206`·`207`·`210` 중 **단 하나라도 지연되면 프로젝트 전체가 1:1로 지연**됩니다. 착수 전 인터페이스 계약(§3.4) 사전 합의를 필수로 둡니다.
...
gantt
    ...
    TASK-212 4영역 성장 나무 & 정체 판정       :crit, t212, 2026-09-04 00:00, 1d
...
gantt
    ...
    TASK-212 4영역 성장 나무 & 정체 판정       :crit, a1t212, 2026-09-04 00:00, 1d
...
| A-4 | 4영역 성장 나무 | 영역별 학습·퀴즈를 채워도 해당 실천 0회면 **해당 나무 승급 불가** / 13일 정체 미판정 / 14일 넛지 출력 | `TASK-302` | `npm run test tests/unit/growth.test.ts` |
...
| **REQ-FUNC-005** 4영역 성장 나무·영역별 3조건·14일 정체 | `212` | `302` | Alpha |
