...
> **문서 구성 안내**
> 본 SRS는 **사내 SRS 표준 양식(1~7장)** 을 그대로 따른다. 요구사항·인수 기준·검증 방식·착수 조건이 **전부 4장 안에** 있어, 개발·테스트·유지보수 사이클에서 **4장 하나만 보면 되도록** 구성했다.
> PRD v1.0에 있으나 표준 양식에 대응 절이 없는 내용은 버리지 않고 **부록 6.5~6.9**에 참조 자료로 두었으며, 각 절에 ISO/IEC/IEEE 29148:2018 근거를 밝혔다.

| 절 | 내용 | 29148 근거 |
| --- | --- | --- |
| 1.2 범위 | In / Out / **제약(Limitations)** | §9.6.3 Scope · §9.6.7 Limitations |
| 4.1 전제조건 및 착수 순서 | **의존성** · 스프린트 배분 · 릴리즈 게이트 | §9.6.8 Assumptions and dependencies · §9.6.9 Apportioning of requirements |
| 4.2 ~ 4.4 요구사항표 | 「인수 기준」에 **Given/When/Then + 실패 케이스**, 「검증 방식」에 **테스트·실험·판정 임계치** 흡수 | §9.6.10 Specified requirements · §9.6.19 Verification |
| 6.5 인앱 이벤트 명세 | 측정 인터페이스 | §9.6.11 External interfaces |
| 6.6 모니터링 및 대응 SLA | 운영 속성 | §9.6.18 Software system attributes |
| 6.7 성과 지표 정의 | 북극성 KPI · MOE · 판정 규칙 | §6.3.3.3 measures of effectiveness · §6.6.3 |
| 6.8 설계 결정 근거 (ADR) | 요구사항 Rationale | §5.2.8 (Rationale 속성) · §9.6.16 Design constraints |
| 6.9 가정 및 리스크 | 미확정 전제와 대응 | §9.6.8 · §5.2.8 (Risk 속성) |
...
| 구분 | 항목 | 사유 |
| --- | --- | --- |
| **영구 제외** | 앱 외부 현금(세뱃돈·친척 용돈) · 친구 기능 · 모의투자 · 미션 사진 인증 · **위치정보 수집 일체** | 제품 정책 · ADR-003 |
...
**제약 (Limitations)** — 본 릴리즈가 해결하지 않는 사용자 문제

아래는 결함이 아니라 결정이다. 각 항목의 재검토 시점을 함께 못박는다.

| 해결하지 않는 문제 | 내용 | 결정 근거 | 재검토 시점 |
| --- | --- | --- | --- |
| 소비 **직전 자동 개입** | 자동 발동 수단이 없다. 사전 수단은 **계획 카드를 먼저 적어야** 동작한다 | **ADR-003** — 못 잡는 대상이 「3분 미만 소비」에서 **「계획을 적지 않은 소비」** 로 이동했다 | 계획 카드 작성률(MOE-07) 4주 실측 후 |
| **온라인 결제 사전 개입** | 대응 수단 0개 | 온라인 결제는 계획 카드를 적을 계기가 **구조적으로 없다** | v1.1 — 7.2절 |
| 「불리기」 **실천 경로** | 학습만 개통, 실천 잠금 | **ADR-006** — REQ-REG-004 법률 검토 대기 | 법률 검토 완료 시 (4.1.1) |
| 학습이 **행동 변화로 이어진다는 연결** | 설계 근거로 서 있고 고객 문제로는 지탱되지 않는다 | 자체 재현 검증이 필요하다 | 실험 E6 · E10 (6.7.4) |
...
| 규제 상수 | 설계 변수가 아닌 고정 제약. `P-nn` · `F-01`로 식별 (4.4절) |
| 3구간 판정 | 모든 KPI·인수 기준·실험에 공통 적용하는 **PASS / HOLD / FAIL** 판정 규칙 (6.7.2) |
| `k/8` 정수 판정 | n=8 인터뷰 지표는 비율이 아니라 **정수 분수**로 판정한다는 규칙 (6.7.3) |
| Cohen's κ | 코더 2인의 판정 일치도 계수. **0.6 이상**을 요구 |
| G / W / T | 인수 기준 표기 — **G**iven(전제) · **W**hen(행위) · **T**hen(판정) |
| AC / E | Acceptance Criteria — 인수 기준 / **E** = 예외·실패 케이스 인수 기준 |
| ADR | Architecture Decision Record — 설계 결정 기록 (6.8) |
...
| 역할 | 이름 / 부서 | 책임 |
| --- | --- | --- |
| 제품기획 (PM) | 유림 / 제품기획팀 | 요구사항 정의 및 우선순위 결정 · **ADR 승인** · 릴리즈 게이트 판정 |
| 정책·법령 담당 | 병윤 / 정책·법령팀 | **규제 요구사항(4.4) 검증** · 법률 검토(예적금 중개업 해당 여부) · 규제 알림 대응 |
| 경쟁·리뷰 담당 | 하영 / 경쟁·리뷰팀 | 대안 대비 벤치마크 분기 재점검 · **대외 표현 정확성 검수** |
| 서비스분석 담당 | 혜원 / 서비스분석팀 | **KPI 산출 및 기준선 실측** · 실험 설계와 3구간 판정 · rubric 코딩 |
| 개발팀 리드 | 개발팀 리드 | 설계 검토 및 승인 · 스프린트 배분(4.1.2) · SLO 재검토 판단 |
...
**주 사용자 특성** — 요구사항의 설계 전제이며, 인수 기준의 인터뷰 표본(n=8)이 이 두 유형으로 구성된다.

| 구분 | H1 유형 (보호자) | H2 유형 (보호자) |
| --- | --- | --- |
| 프로필 | 39세 · 교사 · 자녀 만 8세 · 용돈앱 8개월 사용 · 응답 당일 | 42세 · 자영업(주 6일 · 07~20시) · 자녀 만 9세 · 현금 봉투 3년 · 확인 주 1회 |
| 원하는 것 | 배운 것이 실제 돈 행동으로 이어졌는지를 **짧은 시간에 확인** | 쓰기 전에 한 번 멈추게 하고, 그 선택이 **기록으로 남게** |
...
| 아이 기기 | 전용 태블릿 (집 안) | 키즈워치 |

> 🔴 **두 유형 모두 「전용 스마트폰을 들고 다니는 아이」가 아니다.** ADR-003이 자동 알림 대신 계획 카드를 택한 직접적 근거이며, 계획 카드는 **보호자 폰에서도 작성 가능**해야 한다(REQ-FUNC-007).
...
> **읽는 법** — 요구사항 하나에 필요한 정보가 한 행에 다 있다.
> 「**인수 기준**」은 `G` 전제 · `W` 행위 · `T` 판정으로 적었고, **`E`로 시작하는 항목이 예외·실패 케이스**다. 모든 기능 요구사항은 **예외 케이스를 2개 이상** 갖는다.
> 「**검증 방식**」은 테스트 종류와 **판정 임계치**를 함께 적었다. `E1`~`E11`은 검증 실험 번호이며 설계 상세는 6.7.4에 있다.
> 「**출처**」의 `ADR-nnn`은 그 요구사항이 왜 이렇게 정의됐는지를 가리킨다 — 근거는 6.8에 있다.

### 4.1 전제조건 및 착수 순서

#### 4.1.1 착수 전제조건

아래가 확정되기 전에는 해당 요구사항의 **구현을 착수하지 않는다.** 각 항목은 6.9의 리스크로 추적한다.

| 전제조건 | 확정 방법 | 기한 | 막히는 요구사항 | 추적 |
| --- | --- | --- | --- | --- |
| 🔴 제휴사 **결제내역 API의 업종 코드 체계** | 계약 협의 + 샘플 데이터 매칭 검증 | **S3 종료 전** | **REQ-FUNC-008** · REQ-FUNC-014 · REQ-NF-005 | R5 |
| 🔴 제휴사 **수수료율 · 최소 물량 조건** | 제휴 조건 문의 | **S3 종료 전** | REQ-NF-021 · REQ-NF-022 (손익 판정) | R4 |
| 🔴 예적금 **중개업 해당 여부** (P-20) | 법률 검토 | **S6 착수 전** | **REQ-FUNC-015** · REQ-REG-004 | — |
| **3D 에셋 납품** (5종 × 8벌 · 1차 2종 × 4벌) | 외주 발주 — 사양은 ADR-007 | **S3 착수 전** | **REQ-FUNC-006** | R9 |
| **본인인증 SDK · 푸시/SMS 채널 계약** | 벤더 선정 | **S1 착수 전** | REQ-FUNC-001 · REQ-FUNC-012 | — |
| **학습 4주제 원고 · 회고 문장 풀 초기 세트** | 콘텐츠 담당 납품 | **S2 착수 전** | REQ-FUNC-003 · REQ-FUNC-008 | — |

#### 4.1.2 스프린트 배분

2주 · 개발 3인 기준. 의존성 위상 정렬로 경계를 자른다. **Must 9 · Should 5 · Could 3 = 17건 전부 1스프린트 이내**이며, 유일한 초과 후보였던 계획 카드 기능은 **REQ-FUNC-007(작성) / REQ-FUNC-008(대조)** 로 분할했다.

| 스프린트 | 배분 요구사항 | 근거 |
| --- | --- | --- |
| **S1** | REQ-FUNC-002 (별 엔진) · REQ-FUNC-001 (온보딩·동의) | 둘 다 의존성 0. 별 엔진은 6개 요구사항의 선행이고, 동의 게이트는 규제 게이트라 가장 먼저 테스트 가능해야 한다 |
| **S2** | REQ-FUNC-003 (학습) · REQ-FUNC-004 (미션) · REQ-FUNC-007 (계획 카드 작성) | 별 엔진 완료 후 착수. 계획 카드 작성은 **결제 연동 없이 단독 동작**한다 |
| **S3** | REQ-FUNC-005 (성장 나무) · REQ-FUNC-006 (아바타·옷장) | 학습·별 완료가 선행. **여기서 보호자 화면이 처음 의미를 가진다** |
| **S4** | REQ-FUNC-008 (계획↔실제 대조) · REQ-FUNC-009 (월간 숲) | 제휴사 결제내역 API 연동 필요 — **4.1.1의 업종 코드 체계가 S3 종료 전 확정돼야 한다** |
| **S5** | REQ-FUNC-010 · 011 · 012 · 013 · 014 *(각 0.5스프린트 · 2건씩 병렬)* | 전부 Should |
| **S6+** | REQ-FUNC-015 · 016 · 017 *(Could)* | REQ-FUNC-015는 법률 검토 통과가 착수 조건 |

#### 4.1.3 릴리즈 게이트

전 항목 **AND 조건**이다. 게이트를 우회하는 예외는 두지 않는다.

| 단계 | 범위 | 시점 | 진입 게이트 |
| --- | --- | --- | --- |
| **α 내부** | 팀 + 지인 가정 3~5 | S4 종료 | ① **REQ-REG 전 항목 자동 테스트 100% 통과** ② 별 원장 불일치 **0건**(REQ-NF-008) ③ 위치 권한 선언 **0건**(REQ-NF-017) ④ REQ-NF-001~003 SLO 충족 |
| **β 클로즈드** | 모집 8슬롯 가정 | S5 종료 | ① **E4 PASS** — 7일 내 첫 실천 인정률 **≥ 60%** ② **E1 PASS** — 나무 5초 회상 **≥ 6/8** ③ **WPA ≥ 5/8** ④ **E7 PASS** — 결제 매칭 정확도 **≥ 90%** ⑤ α 게이트 전 항목 유지 |
| **일반 공개** | 1차 타깃 세그먼트 | β +4주 | ① **WPA ≥ 55%** 2주 연속 ② 아이 정지→보호자 인지 **≤ 3일** ③ 정합성 오류 **0건** ④ **E3 PASS** — 계획 카드 작성률 **≥ 50%** ⑤ 카드 연결률 **≥ 60%** |

> **게이트 미달 시** — 6.7.2 3구간 규칙을 적용한다. **HOLD면 표본 +4 후 재판정**하며 그 사이 다음 단계로 넘어가지 않는다. **FAIL이면 해당 실험의 지정 조치**(6.7.4)를 실행하고 재판정한다.

### 4.2 기능 요구사항

| ID | 제목 | 출처 | 우선순위 | 유형 | 검증 방식 | 인수 기준 | 상태 | 담당자 |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| **REQ-FUNC-001** | 보호자 온보딩 5단계 및 법정대리인 동의 게이트 | PRD §4-1 F7 | Must Have | Functional | 1) 동의 게이트 차단 자동 테스트<br>2) 온보딩 재개 단위 테스트<br>3) 외부 API 실패 복구 테스트<br>4) `onboarding_step` 퍼널 분석 | **AC1** `G` 온보딩 5단계 중 2단계까지 진행 · `W` 앱 종료 후 다음 날 재진입 · `T` **직전 단계에서 이어서 시작** · 재입력 항목 **0건**<br>**AC2** `G` 온보딩 시작 · `W` 완주까지 소요 측정 · `T` 세션 분할 시 총 소요 **중위 ≤ 10분** · **3단계 이탈률 ≤ 30%**<br>**AC3** `G` 가입·본인인증 완료 · `W` 동의 미완 상태로 아이 계정 진입 시도 · `T` **차단률 100%** (REQ-REG-001)<br>**E1** `G` 카드 신청 단계에서 외부 API 실패 · `W` 오류 반환 · `T` **입력값 24시간 보존** · 재진입 시 재입력 **0건** · 오류 사유를 사용자 언어로 표시<br>**E2** `G` 온보딩 중 세션 만료 · `W` 재로그인 · `T` **직전 완료 단계에서 재개**하되 **동의 단계는 재확인**(동의는 캐시하지 않음) | Approved | 개발 엔지니어 |
| **REQ-FUNC-002** | 별 지급 엔진 (트리거 8종 · 차감형 · 초기화 없음) | PRD §4-1 F4 · **ADR-001 · ADR-008** | Must Have | Functional | 1) 트리거별 지급 단위 테스트<br>2) 멱등성 중복 지급 테스트<br>3) 원장 이중 기입 정합성 검증(일일 배치)<br>4) 정적 분석 — 전환 함수 부재 | **AC1** `G` 트리거 8종 중 하나 발생 · `W` 지급 조건 충족 · `T` 6.2 `StarTrigger` 정의대로 지급 — 자동 7종은 즉시, **미션(트리거 4)만 보호자 승인 후**<br>**AC2** `G` 아이가 출석체크 · `W` 조건 없이 접속 · `T` **별 1개 지급**(학습 진도 조건 없음) · 단 **WPA 분자에는 산입하지 않는다**<br>**AC3** `G` 주기가 초기화됨 · `W` 별 잔액을 조회 · `T` **잔액이 유지된다**(별은 초기화하지 않음)<br>**AC4** `G` 임의의 별 증감 · `W` 원장에 기입 · `T` **현금 충전·환급·양도·저금통 전환 경로가 코드에 존재하지 않는다**(REQ-REG-005)<br>**E1** `G` 아동 기기 오프라인 · `W` 실천 완료 후 재연결 · `T` **중복 지급 0건**(`idempotency_key`) · 반영 **≤ 60초** · 주차 귀속은 **`client_ts` 기준**<br>**E2** `G` 동일 미션 승인 요청 2회 이상 · `W` 보호자가 각각 승인 · `T` **별은 1회만 지급** · 원장 불일치 **0건**<br>**E3** `G` 별 잔액 미만의 구매 시도 · `W` 차감을 실행 · `T` **차감하지 않고 거부**하며 잔액이 음수가 되지 않는다 | Approved | 개발 엔지니어 |
| **REQ-FUNC-003** | 학습 커리큘럼 4주제 및 퀴즈 | PRD §4-1 F3 · **ADR-006** | Must Have | Functional | 1) 주제 커버리지 검증<br>2) 퀴즈 채점 단위 테스트<br>3) 잠금 상태 화면 검수<br>4) 인터뷰 Q2-3 | **AC1** `G` 커리큘럼 구성 · `W` 주제 목록을 조회 · `T` 벌기·잘쓰기·모으기·불리기 **4주제 전부 금융 과목**(4/4 = 100%)<br>**AC2** `G` 아이가 학습·퀴즈 수행 · `W` 완료 처리 · `T` 주제별 **완주 여부와 퀴즈 정답 수**가 기록되고 나무 승급 조건 입력으로 전달된다<br>**AC3** `G` 「불리기」 주제 · `W` 아이가 진입 · `T` **학습·퀴즈는 열리고 실천 트리거는 잠긴다**<br>**E1** `G` 「불리기」 실천이 잠긴 상태 · `W` 아이가 실천을 시도 · `T` **"곧 열려요" 안내**를 표시하고, 4영역 미개통을 **결함으로 인식한 응답자 ≤ 2/8**<br>**E2** `G` 퀴즈 오답 후 재시도 · `W` 정답 처리 · `T` **동일 문항의 별은 1회만** 지급한다(반복 정답으로 별을 누적할 수 없음) | Approved | 콘텐츠 담당 |
| **REQ-FUNC-004** | 미션 루프 (조건·금액 사전 설정 → 보호자 승인 → 별 1개) | PRD §4-1 F2 | Must Have | Functional | 1) 미션 CRUD 테스트<br>2) 승인·거절 상태 전이 테스트<br>3) 화면 검수 — 상태 시각 구별<br>4) `approval_state_changed` 분포 확인 | **AC1** `G` 보호자가 미션을 생성 · `W` 조건과 금액을 사전 설정 · `T` 아이 화면에 조건이 **그대로 노출**된다(조건이 보호자 머릿속에만 있지 않다)<br>**AC2** `G` 아이가 미션 완료 · `W` 보호자가 승인 · `T` **별 1개 지급** · 실천 카운트 **+1** · 나무 「벌기」 영역에 반영<br>**AC3** `G` 아이 화면 · `W` 승인 대기 상태를 확인 · `T` **「대기 중」이 「미실천」과 시각적으로 구별**된다(색·문구 상이)<br>**E1** `G` 보호자가 미션을 **거절** · `W` 거절 확정 · `T` **별 미지급** · **「승인되지 않음 + 사유」** 표시 · **「미실천」과 시각 구별** · 실천 카운트 **미가산**<br>**E2** `G` 48시간 초과 미승인 · `W` 대기 상태 유지 · `T` REQ-FUNC-011의 소급 경로로 이관되며 **아이 화면에서 실패로 표시하지 않는다** | Approved | 개발 엔지니어 |
| **REQ-FUNC-005** | 성장 나무 (4영역 · 실천 근거 노출 · 정체 원인 표시) | PRD §4-1 F1 · **ADR-002** | Must Have | Functional | 1) 승급 조건 단위 테스트<br>2) 정체 판정 오탐 테스트<br>3) 5초 노출 회상 테스트 — **E1 PASS ≥ 6/8**<br>4) 실천 도달 인터뷰 — **E2 PASS ≥ 6/8**<br>5) 오귀인·불신 발화 코딩 — **E5 PASS 각 ≤ 2/8** (κ ≥ 0.6)<br>6) 월말 집계 · 화면 검수 | **AC1** `G` 이번 달 실천 1건 이상인 계정 · `W` 나무를 **5초만** 노출 후 덮고 기억나는 것을 질문 · `T` *"이번 달 행동이 어떻게 달라졌나"* 를 **1문장으로 회상 ≥ 6/8**<br>**AC2** `G` 나무 상세 미전개 상태 · `W` *"나무가 자랐다는 건 아이가 뭘 했다는 뜻인가"* 질문 · `T` **진행자 개입 0회로 「실천」에 도달 ≥ 6/8**. *"퀴즈를 많이 풀어서"* 만 나오면 실패<br>**AC3** `G` 학습·퀴즈 조건을 초과 충족하고 **실천 0건**인 아동 · `W` 승급을 판정 · `T` **단계가 상승하지 않는다** · 화면에 **"실천 N회 남음"** 명시<br>**AC4** `G` 학습 3회 + 퀴즈 5개 + **실천 1회 이상** 충족 · `W` 승급을 판정 · `T` 다음 단계로 상승(씨앗 → 새싹 → 나무) · **매달 조건을 상승시키지 않는다**<br>**AC5** `G` 운영 1개월 · `W` 월말 집계 · `T` **4영역 중 2영역 이상 성장한 가정 ≥ 50%** · **4영역 전부 씨앗 정체 가정 ≤ 15%**<br>**AC6** `G` 특정 영역이 **14일 이상 미상승** · `W` 보호자가 해당 영역을 진입 · `T` **정체 원인을 조건 단위로 표시** · **열람률 ≥ 60%**<br>**AC7** `G` 정체 상황 · `W` *"무슨 일이 있었다고 생각하느냐"* 질문 · `T` **오귀인 발화 ≤ 2/8** · **앱 불신 발화 ≤ 2/8**<br>**AC8** `G` 정체 원인을 본 보호자 · `W` 익주 동일 화면 재방문 · `T` **정체 계정의 익주 재방문율 ≥ 50%**<br>**AC9** `G` 승인 대기 1건 이상 · `W` 보호자가 나무를 진입 · `T` **「승인 대기 N건」 표시**(*"이번 주엔 안 했구나"* 라는 반대 결론 차단)<br>**E1** `G` 이번 달 실천 기록 **0건** · `W` 나무를 연다 · `T` **"아직 기록이 없어요 + 첫 실천 안내"** 표시 · **빈 화면을 앱 결함으로 인식한 응답자 ≤ 2/8**<br>**E2** `G` 정체 원인이 **복수**(학습·실천 둘 다 미충족) · `W` 원인을 표시 · `T` **미충족 조건 전부 표시** + **「가장 적게 남은 조건」을 최상단**<br>**E3** `G` **주기 초기화 직후**의 미충족 상태 · `W` 정체를 판정 · `T` **「정체」로 표시하지 않는다** — 판정은 **주기 시작 후 14일 경과분에만** 적용 · **오탐 0건** | Approved | 개발 엔지니어 |
| **REQ-FUNC-006** | 아바타 및 옷장 (사전 제작 3D · 5종 × 8벌) | PRD §4-1 F5 · **ADR-007** | Must Have | Functional | 1) 구매 차감 단위 테스트<br>2) 에셋 조합 렌더 검증<br>3) 재접속률 코호트 관측 (목표 3일 ≥ 65% / 7일 ≥ 50%) | **AC1** `G` 에셋 납품 완료 · `W` 옷장을 조회 · `T` **동물 5종 × 옷 8벌 = 40조합** 제공(1차 납품 시점에는 2종 × 4벌)<br>**AC2** `G` 별 잔액이 옷 가격 이상 · `W` 구매 실행 · `T` **별 차감 후 즉시 착용 가능**하며 원장에 차감이 기입된다<br>**AC3** `G` 여러 주기에 걸쳐 모은 별 · `W` 고가 아이템을 구매 · `T` **구매 가능**(별은 주기 초기화되지 않으므로)<br>**E1** `G` 별 잔액 부족 · `W` 구매 시도 · `T` **구매를 차단**하고 **부족한 별 수를 표시**한다<br>**E2** `G` 1차 납품(2종 × 4벌)만 반영된 상태 · `W` 아이가 옷장을 연다 · `T` **미납품 조합을 빈칸으로 노출하지 않고** 납품분만 표시한다 | Approved | 개발 엔지니어 |
| **REQ-FUNC-007** | 소비 계획 카드 작성 (어디서 · 업종 · 얼마까지 · 무엇을) | PRD §4-1 F8a · **ADR-003** | Must Have | Functional | 1) 입력·저장 단위 테스트<br>2) 업종 코드 대조 가능성 검증<br>3) 작성률 4주 실측 — **E3 PASS ≥ 50%** (`plan_card_created ÷ payment_settled`) | **AC1** `G` 아이 또는 보호자 계정 · `W` 계획 카드를 작성 · `T` **어느 쪽이든 저장 가능**하며 **기기 종류와 무관**하다(위치 권한·푸시 권한을 요구하지 않는다)<br>**AC2** `G` 계획 카드 작성 화면 · `W` 장소·업종·금액 상한을 입력(품목은 선택) · `T` 저장되고 **업종은 카드 승인 데이터의 업종 코드와 대조 가능한 값**으로 기록된다<br>**AC3** *(생존 조건)* `G` 운영 4주 · `W` 작성률을 집계 · `T` **카드 결제 건 대비 계획 카드 작성률 ≥ 50%**<br>**E1** `G` 업종을 선택하지 않고 저장 시도 · `W` 저장 실행 · `T` **저장을 거부**하고 업종이 필수임을 안내한다(업종 없는 카드는 대조 불가)<br>**E2** `G` 계획 카드 작성 후 **결제 없이 유효기간 경과** · `W` 만료 배치 실행 · `T` 상태를 **`EXPIRED`** 로 전이하고 **작성률 분모에서 제외하지 않는다**(작성 자체는 집계에 남긴다) | Approved | 개발 엔지니어 |
| **REQ-FUNC-008** | 계획↔실제 업종별 대조 · 두 갈래 판정 · 회고 문장 풀 | PRD §4-1 F8b · **ADR-003 · ADR-004** | Must Have | Functional | 1) 결제 매칭 수동 대조 표본 50건/주 — **E7 PASS ≥ 90%**<br>2) 두 갈래 판정 단위 테스트<br>3) 문장 비복원 추출 검증<br>4) 회고 체류·재노출 로그 — **E8 PASS** p50 ≥ 3초 · 재노출 ≤ 2/8<br>5) 화면 검수 · 인터뷰 Q1-15 | **AC1** `G` 계획 카드가 존재하는 아동 · `W` 카드 결제 발생 · `T` **자동 매칭**되며 **정확도 ≥ 90%**<br>**AC2** *(갈래 A)* `G` 실제 결제 합계 **≤** 계획 금액 · `W` [확인했어요] 선택 · `T` **별 1개 지급** · `plan_met=true` · `star_granted=true`<br>**AC3** *(갈래 B)* `G` 실제 결제 합계 **>** 계획 금액 · `W` [확인했어요] 선택 · `T` **회고 문장은 동일하게 제시**하되 **별 미지급** · `plan_met=false` · **보유 별을 차감하지 않는다**<br>**AC4** `G` 회고 화면 진입 · `W` 확인 버튼까지 진행 · `T` **체류 중위 ≥ 3초** · 체류 1초 미만 **≤ 20%**<br>**AC5** `G` 7일 내 회고 3회 이상 수행 · `W` 문장을 배정 · `T` **비복원 추출** · **동일 문장 재노출률 ≤ 2/8**<br>**AC6** `G` 계획 준수일과 초과일 · `W` 각각 회고를 완료 · `T` **문장이 서로 구별**되고 보호자가 화면만 보고 두 경우를 **구분 ≥ 6/8**<br>**AC7** `G` 갈래 B(별 미지급) 건 · `W` 회고가 대기 · `T` **회고 열람률이 갈래 A 대비 ≥ 70%**(보상 없는 쪽 회고가 방치되면 실패한 날의 데이터만 비게 된다)<br>**E1** `G` 계획 카드 **없이** 결제 발생 · `W` 아이가 앱을 연다 · `T` **대조 화면 대신 「다음엔 가기 전에 적어볼까요?」 유도** 노출 · **별 미지급**<br>**E2** `G` 한 계획 카드에 **여러 건의 결제** 매칭 · `W` 대조 화면 생성 · `T` **합계로 판정**하고 업종별 내역을 모두 나열<br>**E3** `G` 계획 업종과 **다른 업종에서만** 결제(금액은 계획 이내) · `W` 대조 화면 생성 · `T` **별 1개 지급**(금액 기준 단독 판정) · `plan_met=true` · `category_met=false` · 회고는 **「업종이 달랐어요」 갈래**로 분기<br>**E4** `G` 업종 코드가 **`UNKNOWN`** 으로 수신 · `W` 매칭 시도 · `T` **가맹점명 문자열 2차 매칭** → 실패 시 **금액 단독 판정** · `match_method` 적재로 **정확도 산출에서 분리 집계**<br>**E5** `G` 회고 문장 풀 **잔여 20% 이하** · `W` 다음 문장 배정 · `T` **운영 알림 발송** · 재사용 시작 전 **풀 확장 요구**<br>**E6** `G` 회고 미완 상태에서 **다음 소비 발생** · `W` 아이가 앱을 연다 · `T` **큐에 쌓아 순서대로 제시** · 큐 **3건 초과 시 오래된 건을 「요약 회고」로 병합** | Approved | 개발 엔지니어 |
| **REQ-FUNC-009** | 월간 숲 (전월 대비 변화 지표) | PRD §4-1 F9 · **ADR-002** | Must Have | Functional | 1) 스냅샷 적재 단위 테스트<br>2) 델타 렌더 검증<br>3) 60초 지목 인터뷰 Q1-10<br>4) `forest_view_opened.dwell_ms` p50 관측 | **AC1** `G` 주기 종료 · `W` 스냅샷 적재 · `T` **초기화 없이 누적**되며 4영역 최종 단계·집계·총 획득 별이 보존된다<br>**AC2** `G` 전월 데이터 존재 · `W` *"지난달과 비교해 뭐가 달라졌는지 찾아보라"* 요청 · `T` **변화 항목을 60초 이내 3개 이상 지목 ≥ 6/8** · 확인 소요 **중위 ≤ 3분**<br>**AC3** `G` 월간 숲 화면 · `W` 렌더 · `T` **전월 대비 변화 지표 7개**(4영역 단계 + 사려다 멈춤 · 가격 비교 · 저축률) 노출<br>**AC4** `G` 아이가 별을 즉시 소진해 **잔액 0** · `W` 월간 숲을 연다 · `T` **「이번 달 획득 별」이 스크롤 없이 노출**되고 누적 성과를 **지목 가능 ≥ 6/8**<br>**E1** `G` 가입 첫 달로 **전월 데이터 없음** · `W` 월간 숲을 연다 · `T` 비교 영역을 **"다음 달부터 비교할 수 있어요"로 대체** · **델타 0으로 렌더하지 않는다**<br>**E2** `G` 종료된 주기에 **소급 실천이 반영**됨 · `W` 해당 주기 스냅샷을 조회 · `T` **소급분이 그 주기 스냅샷에 반영**되고 *"지난 달 실천으로 인정됐어요"* 를 표시한다 | Approved | 개발 엔지니어 |
| **REQ-FUNC-010** | 아이 온보딩 (학습 → 퀴즈 → 별 1개 → 아이템 제시) | PRD §4-1 F6 | Should Have | Functional | 1) 첫 보상 루프 소요 측정<br>2) 단계 전이 테스트<br>3) 카드 미보유 상태 화면 검수 | **AC1** `G` 아이 계정 최초 진입 · `W` 온보딩 진행 · `T` 짧은 학습 → 퀴즈 → **별 1개 지급** → **별 1개로 살 수 있는 아이템 즉시 제시**까지 **목표 5분** 내 완결<br>**AC2** `G` 카드 배송 대기 상태 · `W` 아이가 앱을 연다 · `T` **학습·퀴즈·별 획득 가능**하고 **카드 필요 기능만 잠긴다**<br>**E1** `G` 온보딩 중 이탈 후 재진입 · `W` 앱을 다시 연다 · `T` **직전 단계에서 재개**하고 이미 받은 별을 **중복 지급하지 않는다**<br>**E2** `G` 온보딩 별 1개로 살 수 있는 아이템이 **1차 납품에 없음** · `W` 아이템을 제시 · `T` **구매 가능한 것만 제시**하고 빈 목록을 노출하지 않는다 | Approved | 개발 엔지니어 |
| **REQ-FUNC-011** | 승인 지연 시 별 소급 지급 및 「승인 대기 N건」 | PRD §4-1 F10 | Should Have | Functional | 1) 소급 지급 단위 테스트 — **성공률 100%**<br>2) 주기 귀속 테스트<br>3) 일괄 승인 테스트<br>4) `approval_state_changed(state=backfilled)` 검증 | **AC1** `G` 미션 완료 후 보호자 미승인 **48시간 경과** · `W` 이후 승인 · `T` **완료 시점 기준으로 소급 지급** · **성공률 100%**<br>**AC2** `G` 승인 대기 1건 이상 · `W` 보호자 화면 렌더 · `T` **「승인 대기 N건」** 표시(REQ-FUNC-005 AC9와 동일 표기)<br>**E1** `G` 완료 시점 주기(N)가 **이미 종료**되고 보호자가 주기 N+1에 승인 · `W` 소급 지급 실행 · `T` **별은 지급**하되 **나무 조건은 주기 N에 귀속** · **N+1 나무에 가산하지 않는다** · **월간 숲(주기 N 스냅샷)에 반영** · *"지난 달 실천으로 인정됐어요"* 표시<br>**E2** `G` 승인 대기 **5건 이상** 누적 · `W` 보호자가 앱을 연다 · `T` **일괄 승인 경로 제공** · 일괄 승인 시에도 **각 건의 완료 시각 기준으로 개별 소급**<br>**E3** `G` 소급 대상 미션이 **거절**됨 · `W` 거절 확정 · `T` **소급 지급하지 않고** REQ-FUNC-004 E1의 거절 처리를 따른다 | Approved | 개발 엔지니어 |
| **REQ-FUNC-012** | 아이 3일 미접속 알림 | PRD §4-1 F11 | Should Have | Functional | 1) 배치 판정 단위 테스트<br>2) 오탐 발송 테스트 — **0건**<br>3) 채널 폴백 테스트<br>4) 발송·열람 로그 — **E9 PASS** 인지 ≤ 3일 · 열람 ≥ 50% | **AC1** `G` 아동 최종 접속 후 **72시간 경과** · `W` 배치 실행 · `T` 보호자에게 **발송률 100%** · **정지→인지 ≤ 3일**<br>**AC2** `G` 미접속 알림 수신 · `W` 보호자가 알림을 연다 · `T` **아이가 멈춘 지점(영역·조건)이 함께 표시**된다<br>**AC3** `G` 보호자가 야간에만 확인 가능 · `W` 발송 시간대를 설정 · `T` **보호자 활동 시간에 맞춰 조정 가능** · 알림 **열람률 ≥ 50%**<br>**E1** `G` 보호자가 **푸시 차단** · `W` 72시간 미접속 판정 · `T` **앱 내 배너 + (동의 시) 문자**로 대체 발송 · **차단 상태 계측** · 인지 일수 산출에서 **별도 집계**<br>**E2** `G` 아동이 **앱을 삭제** · `W` 72시간 판정 시점 도래 · `T` 문구를 **「재설치 안내」로 분기**하고 **다른 이벤트 코드**로 적재<br>**E3** `G` 미접속 **71시간** 시점에 아이가 재접속 · `W` 배치 실행 · `T` **발송하지 않는다** — **오탐 0건** | Approved | 개발 엔지니어 |
| **REQ-FUNC-013** | 위시리스트 (30 · 70 · 100% 각 별 1개) | PRD §4-1 F12 | Should Have | Functional | 1) 진척 구간 지급 테스트<br>2) 중복 지급 방지 테스트<br>3) 실천 트리거 산입 검증 | **AC1** `G` 위시리스트 목표 설정 · `W` 저축 진척이 30 · 70 · 100%에 도달 · `T` **각 구간에서 별 1개 지급** · 실천 트리거 6으로 **WPA 분자에 산입**<br>**AC2** `G` 이미 지급된 구간 · `W` 진척이 오르내림 · `T` **동일 구간은 1회만 지급**<br>**E1** `G` 목표 금액을 **하향 조정**해 이미 30%를 넘긴 상태가 됨 · `W` 진척을 재계산 · `T` **소급 지급하지 않는다**(목표 조정으로 별을 얻을 수 없다)<br>**E2** `G` 위시리스트를 **삭제** · `W` 삭제 실행 · `T` **이미 지급된 별을 회수하지 않는다**(별은 차감형이나 지급 취소는 없다) | Approved | 개발 엔지니어 |
| **REQ-FUNC-014** | 소비 내역 (전월 대비 증감액 상단 · 업종별 집계) | PRD §4-1 F13 | Should Have | Functional | 1) 집계 정확성 테스트<br>2) 업종 분류 검증<br>3) 화면 검수 — 상단 배치 | **AC1** `G` 전월 소비 데이터 존재 · `W` 소비 내역 화면 진입 · `T` **전월 대비 증감액이 화면 상단**에 배치된다<br>**AC2** `G` REQ-FUNC-008이 수집한 업종 코드 · `W` 집계 실행 · `T` **업종별 집계**를 제공한다(별도 입력 없이 재사용)<br>**E1** `G` 업종이 **`UNKNOWN`** 인 결제 · `W` 업종별 집계 · `T` **「미분류」 항목으로 표시**하고 다른 업종에 임의 배분하지 않는다<br>**E2** `G` 전월 데이터 없음 · `W` 화면 진입 · `T` 증감 영역을 **"다음 달부터 비교할 수 있어요"로 대체**(REQ-FUNC-009 E1과 동일 규칙) | Approved | 서비스 운영자 |
| **REQ-FUNC-015** | 예적금 비교·선택 (가입 별 1개 · 완주 별 10개) | PRD §4-1 F15 · **ADR-006** | Could Have | Functional | 1) **법률 검토 통과 확인 (착수 게이트)**<br>2) 중개 행위 부재 검증<br>3) WPA 버전 전환 검증 | **AC1** `G` 법률 검토 통과 · `W` 기능 개통 · `T` **비교·선택 경험까지만** 제공하고 **가입 중개를 수행하지 않는다**(REQ-REG-004)<br>**AC2** `G` 기능 개통 · `W` 실천 트리거 7·8이 열린다 · `T` WPA 정의를 **v1(3종) → v2(5종)** 로 전환하고 **시계열을 단절 표기**하며 전환 후 **4주간 v1·v2를 병기**한다<br>**E1** `G` 법률 검토 **미통과** · `W` 착수를 시도 · `T` **구현을 착수하지 않는다**(4.1.1 전제조건)<br>**E2** `G` 예적금 만기 전 중도 해지 · `W` 해지 처리 · `T` **가입 시 지급한 별 1개는 회수하지 않고** 완주 별 10개는 지급하지 않는다 | Proposed | 정책·법령 담당 |
| **REQ-FUNC-016** | 카드 없이 학습부터 시작하는 체험 경로 | PRD §4-1 F16 | Could Have | Functional | 1) 잠금 범위 테스트<br>2) 전환 흐름 테스트 | **AC1** `G` 카드 미신청 또는 배송 대기 · `W` 아이가 앱을 사용 · `T` **학습·퀴즈·별 획득 가능**하고 **카드 필요 기능만 잠긴다**<br>**AC2** `G` 체험 중 카드 등록 완료 · `W` 전환 실행 · `T` **체험 중 획득한 별과 학습 이력이 보존**된다<br>**E1** `G` 본 기능 미구현 상태 · `W` 카드 없는 아이가 진입 · `T` **잠금 안내 문구로 대체**한다(REQ-FUNC-010 AC2)<br>**E2** `G` 체험 기간 중 카드 신청이 **거절**됨 · `W` 결과 수신 · `T` 체험 상태를 유지하고 **획득분을 삭제하지 않는다** | Proposed | 개발 엔지니어 |
| **REQ-FUNC-017** | 별의 옷장 외 목적지 | PRD §4-1 F17 | Could Have | Functional | 1) **분리선 준수 정적 분석 (착수 게이트)**<br>2) 사용처별 차감 테스트 | **AC1** `G` P-21 현금 분리선 재검토 완료 · `W` 새 사용처 개통 · `T` **별↔저금통 분리선을 넘지 않는다**(REQ-REG-005) — 현금·현금성 재화로 전환되지 않는다<br>**AC2** `G` 복수 사용처 존재 · `W` 아이가 사용처를 선택 · `T` 차감이 **원장에 사용처와 함께 기입**된다<br>**E1** `G` 분리선 재검토 **미완료** · `W` 착수를 시도 · `T` **구현을 착수하지 않는다**<br>**E2** `G` 사용처가 일시 중단됨 · `W` 아이가 진입 · `T` **이미 차감된 별을 복구**하고 중단 사유를 표시한다 | Proposed | 정책·법령 담당 |
| **REQ-FUNC-018** | 기존 앱 기록 이전 / 병행 사용 | PRD §4-1 F18 | Won't Have | Functional | — *(본 릴리즈 미구현)* | **AC1** `G` 본 릴리즈 · `W` 타사 기록 이전을 시도 · `T` **제공하지 않는다.** 획득 메시지를 **「이전」이 아니라 「지금부터」** 프레임으로 구성해 부재를 보완한다 | Deferred | 제품기획 (PM) |
...
> **상위 추적** — 각 요구사항의 PRD 출처와 설계 결정(ADR)은 4장 「출처」 열에, 인수 기준과 검증 방식은 4장 해당 행에 있다. ADR 본문은 6.8절이다.
...
> **확장 절** — ISO/IEC/IEEE 29148:2018 **§9.6.11 External interfaces**. 6.7절의 성과 지표는 이 이벤트 스펙 없이는 산출·검증이 불가능하므로 인터페이스 명세로서 함께 둔다.
...
> **확장 절** — ISO/IEC/IEEE 29148:2018 **§6.3.3.3**(stakeholder needs often include measures of effectiveness) · **§6.6.3 Measurement for requirements**. 4.1.3 릴리즈 게이트와 4.2 「검증 방식」이 참조하는 지표·판정 규칙의 정의를 모아 둔다.

#### 6.7.1 북극성 KPI — WPA (Weekly Practiced Active)
...
| 항목 | 값 |
| --- | --- |
| **분자 산입 트리거** | **WPA-v1 (MVP)** — 트리거 4 미션 승인 · 5 소비 회고 · 6 위시리스트 = **3종**<br>**WPA-v2 (REQ-FUNC-015 개통 후)** — v1 + 트리거 7 · 8 예적금 = **5종** |
| **제외 규칙** | 트리거 1 온보딩 학습 · 2 출석체크 · 3 퀴즈 정답. 셋 다 **학습 경로**이며, 출석 별이 분자에 들어가면 *"접속만으로 실천 지표가 오르는"* 활동량 지표로 퇴화한다 (ADR-001 · ADR-008) |
...
| **측정 경로** | `practice_credited` + 활성 아동 스냅샷 (6.5절) |
| **버전 전환 규칙** | v2 전환 시 **시계열을 단절 표기**하고 전환 후 4주간 v1·v2를 **병기**한다 (REQ-FUNC-015 AC2) |
...
| 회고 체류 중위 (MOE-16) | ≥ 3초 | 2~3초 | < 2초 |
| 결제↔계획 카드 매칭 정확도 (REQ-NF-005) | ≥ 90% | 80~89% | < 80% |
| 아이 정지 → 보호자 인지 (MOE-13) | ≤ 3일 | 4~7일 | > 7일 |
| 카드 연결률 (MOE-18) | ≥ 60% | 40~59% | < 40% |

#### 6.7.3 인터뷰 지표 판정 규칙

4.2절의 인수 기준 중 `≥ 6/8` 같은 **인터뷰 표본 기반 항목**에 적용한다.

**① 정수 임계치 환산** — n=8에서 `≥70%`는 5.6명이라 정수 경계가 불명확하다. **모든 인터뷰 인수 기준은 `k/8` 로 표기**한다.
...
4.2절 「검증 방식」의 `E1`~`E11`이 가리키는 실험 설계다. FAIL 조치는 4.1.3 게이트 미달 시 실행한다.

| # | 검증 대상 가설 | 설계 | 측정 도구 | PASS | HOLD | FAIL → 조치 |
| --- | --- | --- | --- | --- | --- | --- |
...
| **E10** 🔴 | **실천 증가가 실제 소비·저축 행동 변화로 이어진다** | **3개월 종단 관찰** — 월간 숲 3지표 | 사려다 멈춤 · 가격 비교 · 저축률 | **3지표 중 2개 이상** 3개월 연속 개선 | 1개만 개선 | 실천은 늘고 3지표 불변 → 🔴 **「성장」 정의 재검토** |
| **E11** | 대안 대비 공백이 유지된다 | **분기 1회 경쟁사 재점검** | 보호자 화면 캡처 · 약관 대조 · 과목 목록 | 공백 유지 | 부분 변화 | 공백이 메워짐 → 차별 축 재산정 |

#### 6.7.5 보조 KPI (Measures of Effectiveness)

`⬜` 는 **기준선 실측 전**이라는 뜻이며 목표가 미정이라는 뜻이 아니다.

| ID | Outcome | 지표 | 기준선 | **목표** | 기준선 확보 | 주기 | 측정 창구 |
| --- | --- | --- | --- | --- | --- | --- | --- |
| **MOE-01** | 변화가 한 문장으로 읽힌다 | 나무 5초 노출 후 1문장 회상 성공률 | ⬜ | **≥ 6/8** | 인터뷰 1회차 | 인터뷰 회차 | 인터뷰 T3 rubric (κ ≥ 0.6) |
...
> **MOE-18 목표값 산출 근거** — 페르소나 충전액 중위(주 6,500원 × 4.3주 ≈ **28,000원**)의 소진율 100%를 상한으로 잡고 30,000원을 목표로 둔다. 손익 조건 **`월 결제액 × 우리 몫 수수료율 ≥ 아동 1인당 월 원가`** 의 우변은 4.1.1의 제휴 조건 확정 시 채운다 (REQ-NF-021 · REQ-NF-022).

### 6.8 설계 결정 근거 (ADR)

> **확장 절** — ISO/IEC/IEEE 29148:2018 **§5.2.8**(요구사항 속성 중 **Rationale**) · **§9.6.16 Design constraints**. 4장 「출처」 열의 `ADR-nnn`이 여기를 가리킨다. 결정을 바꾸려면 **새 ADR을 쓰고 이전 것을 Superseded로 표시**한다. 각 ADR의 「검증 연결」은 그 구조가 **어떤 가설을 확인하기 위한 것인지**를 밝힌다 — 구조가 가설 검증을 막으면 그 구조가 틀린 것이다.

| ID | 결정 | 영향 요구사항 | 검증 연결 |
| --- | --- | --- | --- |
| **ADR-001** | 북극성 KPI를 **WPA 단일 지표**로 둔다 | REQ-FUNC-002 · 6.7.1 | E4 · β 게이트 WPA ≥ 5/8 |
...
| **ADR-008** | **출석 별은 조건 없이 지급**하되 WPA에서 제외 | REQ-FUNC-002 · 6.7.1 | E8 (r ≤ 0.3) |
...
> **확장 절** — ISO/IEC/IEEE 29148:2018 **§9.6.8 Assumptions and dependencies** · **§5.2.8**(요구사항 속성 중 **Risk**). 착수를 막는 **의존성**은 4.1.1에, 릴리즈 범위를 정하는 **제약**은 1.2절에 있다. 여기에는 **아직 확인되지 않은 전제와 그 대응**만 남긴다.

#### 6.9.1 가정

| 가정 | 확정 방법 | 기한 | 추적 |
| --- | --- | --- | --- |
| 보호자가 **금융 집중을 원한다** | 반증 인터뷰 H1-3 (실험 E6) | 인터뷰 1회차 | R3 |
| 실천 증가가 **실제 소비·저축 행동 변화로 이어진다** | 3개월 종단 관찰 (실험 E10) | 프로토타입 3개월차 | R2 |
| 보호자가 **미션에 걸린 이자를 실제로 준다** | 인터뷰 검증과제 ③ | 인터뷰 1회차 | — |
| 기회점수 순위가 **우선순위 판단으로 유효하다** | 인터뷰 후 재채점 | 8슬롯 완료 시 | — |

#### 6.9.2 리스크 및 대응

조기 경보 지표는 전부 6.6절 모니터링에 등재돼 있어, **리스크가 지표로 감시**된다.
...
| **R4** 🔴 | **수익 산출식의 우변(원가·수수료율) 미확인** | 손익분기 판정 불가 | 제휴 조건 확정을 **S3 종료 전** 완료(4.1.1) · 그때까지 좌변 목표로 운영 | 카드 연결률 **< 40%** 또는 월 결제액 **< 20,000원** 2개월 연속 |
| **R5** 🔴 | **제휴사 업종 코드 상세도 부족** | REQ-NF-005 미달 → 대조 화면 신뢰 상실 → REQ-FUNC-014까지 연쇄 실패 | **계약 시 코드 체계 명시** + 샘플 데이터 사전 검증 · 가맹점명 2차 매칭 · 금액 단독 폴백(REQ-FUNC-008 E4) | 매칭 정확도 **< 90% 2주 연속** · `amount_only` **> 20%** |
| **R6** | **모집단 연 5~12% 감소** | 시장 분모가 해마다 줄어듦 · 진입 속도 자체가 경쟁력 | 연도 변경 시 세그먼트 재산출 · 스프린트 계획을 **6스프린트(12주) 내로 압축**(4.1.2) | 연 1회 학생 수 추계 갱신 |
...
| **R9** | **3D 에셋 납품 지연이 S3를 막음** | 아이 동기 장치 없이 프로토타입 진입 | **발주를 S1과 동시 착수** · 1차 납품을 **2종 × 4벌**로 축소(REQ-FUNC-006 E2) | 발주 후 6주차 납품 진척 **< 50%** |
...
- 그때까지 대외 표현은 **「설계상 그렇게 되도록 만들었다」** 까지로 제한한다
