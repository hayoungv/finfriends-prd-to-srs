...
- **작성일:** 2026-08-25
- **기준 문서:** [`docs/02-srs/srs.md`](../docs/02-srs/srs.md)
...
| Task ID | 작업 명칭 및 설명 | 규모 | 선행 의존 | 관련 REQ / SRS |
|---|---|:---:|---|---|
| **`TASK-212`** | **[Read/Write] 4영역 성장 나무 영역별 3조건 판정 및 14일 정체(Stall) 평가 Action**<br>• `evaluateGrowthTree`: 슬롯별 `학습>=3 AND 퀴즈>=5 AND 해당 영역 실천>=1` 승급 판정<br>• 사이클 14일 미만 정체 판정 방지 및 14일 경과 시 부족 조건 넛지 산출 | 1.0d | `TASK-206`, `TASK-207`, `TASK-210` | REQ-FUNC-005 |
...
| **`TASK-215`** | **[Write] 부모 용돈 위시리스트 저축 및 30%/70%/100% 마일스톤 별 지급 Action**<br>• `updateWishlistDeposit`: 부모 용돈 저축 원장 누적 시 30/70/100% 단계별 1회 한정 별 지급. 별은 저축액·구매대금으로 사용하지 않음 | 0.5d | `TASK-204` | REQ-FUNC-013 |
...
| **`TASK-302`** | **[Unit] 4영역 성장 나무의 영역별 3조건 승급 및 14일 정체 판정 로직 테스트**<br>• **GWT:** 특정 영역 학습 10회 완료해도 해당 영역 실천 0회면 그 나무 승급 불가, 13일 차에는 정체 미판정, 14일 차에 정체 넛지 출력 검증 | 0.5d | `TASK-212` | REQ-FUNC-005 |
...
- **총 예상 공수:** **20.5 Man-Days (M/D)**

| 릴리즈 게이트 | 포함 태스크 | 통과 기준 (Quality Gate) |
|---|---|---|
| **Alpha Gate** | `Step 1` (101~104)<br>+ `Step 2` 핵심 (201~212)<br>+ `Step 3` (301~305) | • 동의 미완료 아동 차단 100% (REG-001)<br>• 별 원장 멱등성 및 잔액 불변식 100% 통과<br>• 4영역 성장 나무의 영역별 3조건 판정 정상 동작 |
