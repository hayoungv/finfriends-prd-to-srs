...
- `[NEW]` `app/child/learn/page.tsx` — 4주제 카드·불리기 잠금 (Fun)
- `[NEW]` `app/child/quiz/[topic]/page.tsx` — 퀴즈 5단계 전이 (Fun)
- `[NEW]` `app/child/quiz/[topic]/quiz.fixture.ts` — fixture

> fixture 첫 줄에 `// PROTO-DATA: TASK-206` 마커를 넣는다. 구현 완료 시 파일을 삭제하고 `grep -rn "PROTO-DATA: TASK-206"` 가 0건임을 확인한다.
